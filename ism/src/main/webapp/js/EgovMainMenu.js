...
function addTreeNode(parentNode, recursedNodes) {
	for (var i = 0; i < treeNodes.length; i++) {
		...
		if (nodeValues[1] == parentNode) {
			...
			if (hasChildNode) {
				vHtmlCode +=vHtmlCodeBg+"<a href='#'>"+nodeValues[2]+"</a>";
			} else{
				// Start link
				if(recursedNodes.length==1){
				   vHtmlCode +=vHtmlCodeBg+"<a href=javascript:fn_MovePage('" + i + "');>"+nodeValues[2]+"</a>";
				}else{
				   vHtmlCode +=vHtmlCodeBgList+"<a href=javascript:fn_MovePage('" + i + "');>"+nodeValues[2]+"</a>";
				}
			}
